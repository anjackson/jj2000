 ...
 * CVS identifier:
 *
 * $Id: ImgWriterPGX.java,v 1.10 2000/11/16 11:30:45 dsanta Exp $
 *
 * Class:                   ImgWriterPGX
 *
 ...
 * PGX is a custom monochrome file format invented specifically to simplify 
 * the use of VM3A with images of different bit-depths in the range 1 to 31 
 * bits per pixel.
 *
 * <P>The file consists of a one line text header followed by the binary datas.
 * <P><u>Header:</u><br>
 ...
 * <i>bit-depth</i> value).
 *
 * <P> If the data is unsigned, level shifting is applied adding
 * 2^(bit depth - 1)
 *
...
public class ImgWriterPGX extends ImgWriter {
    ...
    /** Used during saturation (2^bitdepth-1 if unsigned,
     * 2^(bitdepth-1)-1 if signed)*/
    int maxVal;
    ...
    /** Whether the data must be signed when writing or not. In the
     * latter case inverse level shifting must be applied */
    boolean isSigned;
    ...
    /** The pack length of one sample (in bytes, according to the
        output bit-depth */
     ...
     *
     * <P>The size of the image that is written to the file is the
     * size of the component from which to get the data, specified by
     * b, not the size of the source image (they differ if there is
     * some sub-sampling).
     *
     ...
     * write.
     *
     * @param n The index of the component from where to get the data.
     *
     * @param isSigned Whether the datas are signed or not (needed only
     *                 when writing header).
     *
     * @see DataBlk
     *
     *
     * */
    public ImgWriterPGX(File out, 
			BlkImgDataSrc imgSrc, 
			int n, boolean isSigned) throws IOException{
        //Initialize
        this.c = n;
        ...
        src = imgSrc;
        w = src.getCompImgWidth(n);
        h = src.getCompImgHeight(n);
        fb = imgSrc.getFixedPoint(n);
        ...
        maxVal = 
            (this.isSigned)?
            (( 1<<(src.getNomRangeBits(n)-1) )-1):
            ((1<<src.getNomRangeBits(n))-1);
        minVal =
            (this.isSigned)?
            (-1 * ( 1<<(src.getNomRangeBits(n)-1) )):
            0;
     ...
     *
     * */
    public ImgWriterPGX(String fname, 
			BlkImgDataSrc imgSrc, 
			int n, boolean isSigned) throws IOException{
        this(new File(fname),imgSrc,n,isSigned);
    }
