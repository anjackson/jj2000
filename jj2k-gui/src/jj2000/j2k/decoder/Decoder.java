 ...
 * CVS identifier:
 *
 * $Id: Decoder.java,v 1.4 2002/08/08 13:59:57 grosbois Exp $
 *
 * Class:                   Decoder
 *
...
public class Decoder implements Runnable {
	...
	String btitle = "";
	    ...
	    // **** Side information parser ****
	    SideInfoRetrieval sir = new SideInfoRetrieval(breader,pl);

            // **** Entropy decoder ****
            try {
                entdec = hd.createEntropyDecoder(sir,pl);
            } catch (IllegalArgumentException e) {
	    ...
	    // **** Unscrambling ****
	    Unscrambling wavUns = new Unscrambling(entdec,pl);
            
            // **** ROI de-scaler ****
            try {
                roids = hd.createROIDeScaler(wavUns,pl,decSpec);
            } catch (IllegalArgumentException e) {
	...
	str = Unscrambling.getParameterInfo();
	if(str!=null) for(i=str.length-1; i>=0; i--) vec.addElement(str[i]);
