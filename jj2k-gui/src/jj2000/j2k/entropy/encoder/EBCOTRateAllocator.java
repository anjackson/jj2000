...
public class EBCOTRateAllocator extends PostCompRateAllocator {
	...
	ProgressWatch pw = FacilityManager.getProgressWatch();
		...
		pw.initProgressWatch(0,cblkToEncode,"Encoding tile "+t+"...");
		    ...
		    if(ccb.scrambled) {
			cblkProtOver += Scrambling.MIN_SIDE_INFO_LEN;
			if(ccb.scramblingType==ScramblingTypes.
			   COMP_DOMAIN_SCRAMBLING) {
			    cblkProtOver += 2;
			}
			if((ccb.seed&0xff)==0xff) {
			    cblkProtOver += 1;
			}

			if(isEncryptionNeeded) {
			    cblkProtOver += (rsaMod.bitLength()+7)/8;
			} else {
			    cblkProtOver += 4; // Encode an integer
			}
		    }
	...
	// +------------------------------+
	// Bit stream is simulated to know tile length
	...
	// Find layer index from which to secure
	int secNb = (int)(secBr*src.getImgHeight()*src.getImgWidth()/8.0)-
	    estimHeadOver;
// 	System.out.println("secNb="+secNb+", estimHeadOver="+estimHeadOver);
	secLyStart = numLayers;
	for(int l=0; l<numLayers; l++) {
// 	    System.out.println("Layer "+l+": "+layers[l].actualBytes);
	    if(layers[l].actualBytes>secNb) {
		secLyStart = l;
		break;
	    }
	}

	// Scramble needed layers
 	pktEnc.scrambleLayersFrom(secLyStart,numLayers,
				  cblks,truncIdxs,numPrec);

	// Add termination segments to scrambled code-blocks
	pktEnc.addSecureTermination(numLayers-1,cblks,numPrec);
	// +----------------------------+
	// | Recompute the tile lengths |
	// +----------------------------+
	pktEnc.reset();
	for(int t=0; t<nt; t++) tileLengths[t] = 0;
	for(int l=0; l<numLayers; l++) { // loop on layers
	    rdThreshold = layers[l].rdThreshold;
	    for(int t=0; t<nt; t++) { // loop on tiles
		if(l==0) {
		    // Tile header
		    headEnc.reset();
